...
public interface CreditorClaimRepository extends JpaRepository<CreditorClaim, Long>, JpaSpecificationExecutor<CreditorClaim> {
    ...
    @Query("SELECT c.caseId, SUM(c.totalAmount) FROM CreditorClaim c GROUP BY c.caseId ORDER BY SUM(c.totalAmount) DESC")
    List<Object[]> sumTotalAmountByCaseIdGroup();

    @Query("SELECT c.caseId, c.caseName, SUM(c.totalAmount) FROM CreditorClaim c GROUP BY c.caseId, c.caseName ORDER BY SUM(c.totalAmount) DESC")
    List<Object[]> sumTotalAmountByCaseIdWithNameGroup();

    @Query("SELECT c.id, c.creditorName, c.totalAmount FROM CreditorClaim c ORDER BY c.totalAmount DESC")
    List<Object[]> findTopClaimsByAmount();

    @Query("SELECT c.id, c.creditorName, c.totalAmount FROM CreditorClaim c ORDER BY c.totalAmount DESC")
    List<Object[]> findTopClaimsByAmount(org.springframework.data.domain.Pageable pageable);

    @Query("SELECT c.caseId, c.caseName, SUM(c.totalAmount) FROM CreditorClaim c WHERE c.caseId IN (SELECT bc.id FROM BankruptCase bc WHERE bc.createUserId = :userId) GROUP BY c.caseId, c.caseName ORDER BY SUM(c.totalAmount) DESC")
    List<Object[]> sumTotalAmountByCaseIdWithNameGroupByUserId(@Param("userId") Long userId);

    @Query("SELECT c.id, c.creditorName, c.totalAmount FROM CreditorClaim c WHERE c.caseId IN (SELECT bc.id FROM BankruptCase bc WHERE bc.createUserId = :userId) ORDER BY c.totalAmount DESC")
    ...
    Page<CreditorClaim> findAll(Pageable pageable);

    // 兼容方法，等同于 findAll
    default Page<CreditorClaim> findAllActive(Pageable pageable) {
