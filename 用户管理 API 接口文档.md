...
**请求示例**:
```bash
curl -X POST "http://localhost:5779/api/v1/auth/change-password" \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
    "oldPassword": "oldpass123",
...
**请求示例**:
```bash
curl -X PUT "http://localhost:5779/api/v1/auth/profile/real-name" \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
    "realName": "张三"
...
3. **修改密码**需要提供原密码进行验证
4. **删除操作**是逻辑删除，会将 `is_deleted` 标记为 `true`，`status` 设置为 `DELETED`
5. **更新状态为 LOCKED 或 INACTIVE**时，会自动撤销该用户的所有 Token
6. **手机号和邮箱**具有唯一性，更新时会检查是否已被其他用户使用
7. **更新手机号**时可以选择提供短信验证码进行验证
8. **批量更新状态**时，如果部分用户不存在或已删除，会跳过这些用户继续处理
9. **查询接口**默认过滤掉已删除的用户（`is_deleted = false`）
10. **密码长度**必须在 6-20 位之间，且必须包含字母和数字
11. **手机号和邮箱**不需要进行格式校验
...
- ✅ `PUT /auth/profile/real-name` - 用户修改自己的真实姓名
- ✅ `POST /auth/change-password` - 用户修改自己的密码（需要原密码）
