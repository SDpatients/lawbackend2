...
CREATE TABLE `tb_case_process_stage` (
  ...
  `module_code` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '模块编码',
  `module_name` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '模块名称',
  ...
  KEY `idx_stage_num` (`stage_num`),
  KEY `idx_module_code` (`module_code`),
  KEY `idx_status` (`status`),
  KEY `idx_create_time` (`create_time`),
  CONSTRAINT `fk_case_process_stage_case` FOREIGN KEY (`case_id`) REFERENCES `tb_bankrupt_case` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='案件流程阶段表';
...
| status | varchar(20) | DEFAULT 'ACTIVE' | 状态: ACTIVE-激活, INACTIVE-停用, DELETED-删除 |
| create_time | datetime | DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| update_time | datetime | DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 修改时间 |
| create_user_id | bigint | DEFAULT NULL | 创建者ID |
| update_user_id | bigint | DEFAULT NULL | 修改者ID |
| is_deleted | tinyint(1) | DEFAULT '0' | 是否删除: 0-否, 1-是 |
...
@Data
@TableName(value = "tb_case_process_stage", autoResultMap = true)
...
```sql
-- 阶段一：立案阶段 - 立案信息
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 1, '立案阶段', 'STAGE1_REGISTER_INFO', '立案信息', '案件立案', '案件基本信息立案', '2023-01-10 10:00:00', '[{"fileName": "立案申请书.pdf", "filePath": "/uploads/20230110/立案申请书.pdf", "fileType": "pdf", "fileSize": 1024000}]', '{"registerNo": "L20230110001", "registerDate": "2023-01-10", "caseType": "破产清算", "applicant": "张三", "respondent": "XX公司"}', 'ACTIVE', 1);

-- 阶段二：接管阶段 - 财产接收
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 2, '接管阶段', 'STAGE2_PROPERTY_RECEIPT', '财产接收', '财产接收记录', '接收XX公司财产', '2023-02-15 14:30:00', '[{"fileName": "财产清单.xlsx", "filePath": "/uploads/20230215/财产清单.xlsx", "fileType": "xlsx", "fileSize": 2048000}]', '{"receiver": "李四", "receiveDate": "2023-02-15", "propertyType": "固定资产", "propertyValue": 10000000}', 'ACTIVE', 1);

-- 阶段三：调查阶段 - 财产调查
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 3, '调查阶段', 'STAGE3_PROPERTY_INVESTIGATION', '财产调查', '财产调查报告', 'XX公司财产调查结果', '2023-03-20 09:15:00', '[{"fileName": "财产调查报告.pdf", "filePath": "/uploads/20230320/财产调查报告.pdf", "fileType": "pdf", "fileSize": 3072000}]', '{"investigator": "王五", "investigateDate": "2023-03-20", "investigateResult": "正常", "totalValue": 15000000}', 'ACTIVE', 1);

-- 阶段四：债权阶段 - 债权确认
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 4, '债权阶段', 'STAGE4_CLAIM_CONFIRMATION', '债权确认', '债权确认书', '确认债权人债权', '2023-04-25 11:00:00', '[{"fileName": "债权确认书.pdf", "filePath": "/uploads/20230425/债权确认书.pdf", "fileType": "pdf", "fileSize": 1536000}]', '{"creditorName": "赵六", "claimAmount": 500000, "claimStatus": "确认", "confirmDate": "2023-04-25"}', 'ACTIVE', 1);

-- 阶段五：重整阶段 - 重整计划
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 5, '重整阶段', 'STAGE5_REORGANIZATION_PLAN', '重整计划', '重整计划草案', 'XX公司重整计划', '2023-05-30 16:45:00', '[{"fileName": "重整计划草案.pdf", "filePath": "/uploads/20230530/重整计划草案.pdf", "fileType": "pdf", "fileSize": 4096000}]', '{"planName": "XX公司重整计划", "planContent": "详细重整计划内容", "draftDate": "2023-05-30", "approveStatus": "待批准"}', 'INACTIVE', 1);

-- 阶段六：分配阶段 - 破产财产分配方案
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 6, '分配阶段', 'STAGE6_BANKRUPTCY_DIST_PLAN', '破产财产分配方案', '财产分配方案', 'XX公司财产分配方案', '2023-06-15 10:30:00', '[{"fileName": "财产分配方案.pdf", "filePath": "/uploads/20230615/财产分配方案.pdf", "fileType": "pdf", "fileSize": 2560000}]', '{"planName": "XX公司财产分配方案", "totalAssets": 12000000, "distributionPrinciple": "按比例分配"}', 'INACTIVE', 1);

-- 阶段七：终结阶段 - 注销登记
INSERT INTO `tb_case_process_stage` (`case_id`, `stage_num`, `stage_name`, `module_code`, `module_name`, `title`, `content`, `process_date`, `attachments`, `field_data`, `status`, `create_user_id`) VALUES
(1, 7, '终结阶段', 'STAGE7_CANCELLATION_REG', '注销登记', '公司注销登记', 'XX公司注销登记', '2023-07-20 15:20:00', '[{"fileName": "注销登记申请书.pdf", "filePath": "/uploads/20230720/注销登记申请书.pdf", "fileType": "pdf", "fileSize": 1280000}]', '{"cancelType": "破产注销", "applyDate": "2023-07-20", "approvalDate": null, "approvalStatus": "待批准"}', 'INACTIVE', 1);
```
...
```sql
-- 查询案件1的所有阶段数据
SELECT * FROM `tb_case_process_stage` WHERE `case_id` = 1 AND `is_deleted` = 0;

-- 查询案件1的阶段2数据
SELECT * FROM `tb_case_process_stage` WHERE `case_id` = 1 AND `stage_num` = 2 AND `is_deleted` = 0;

-- 查询案件1的财产接收模块数据
SELECT * FROM `tb_case_process_stage` WHERE `case_id` = 1 AND `module_code` = 'STAGE2_PROPERTY_RECEIPT' AND `is_deleted` = 0;
```
