...
**请求示例**:
```
GET /api/work-log/list?pageNum=1&pageSize=10&caseId=1&workType=CASE_INVESTIGATION&startDate=2026-01-01&endDate=2026-01-31
```
...
const getWorkLogList = async (params) => {
  ...
  const response = await fetch(`/api/work-log/list?${queryParams}`, {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });
